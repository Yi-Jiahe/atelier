  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full">
        <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-[8px] w-full">
          {thumbnails.map((thumbnail: ArtworkThumbnail, index: number) => {
            return (
              <a key={index} href={`artwork/${thumbnail.artworkKey}`} rel="noopener noreferrer">
                <img src={`${thumbnail.href}?width=400&height=400`} alt={thumbnail.alt} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </a>
            );
          })}
        </div>
